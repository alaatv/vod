...
class YaldaCouponSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        for ($i = 0; $i < $fortyPercent; $i++) {
            CouponRepo::createBasicOveralCoupon(config('constants.EVENTS.COUPON').'-'.$i.'40', 40,
                config('constants.EVENTS.COUPON'), null, null, null, null, 0, 0);
        }

        //discountCode = 45%    count=30000
        $thirtyPercent = $totalAmount * 0.3;
        for ($i = 0; $i < $thirtyPercent; $i++) {
            CouponRepo::createBasicOveralCoupon(config('constants.EVENTS.COUPON').'-'.$i.'45', 45,
                config('constants.EVENTS.COUPON'), null, null, null, null, 0, 0);
        }
        ...
        for ($i = 0; $i < $fiftyPercent; $i++) {
            CouponRepo::createBasicOveralCoupon(config('constants.EVENTS.COUPON').'-'.$i.'50', 50,
                config('constants.EVENTS.COUPON'), null, null, null, null, 0, 0);
        }
        ...
        for ($i = 0; $i < $fifteenPercent; $i++) {
            CouponRepo::createBasicOveralCoupon(config('constants.EVENTS.COUPON').'-'.$i.'55', 55,
                config('constants.EVENTS.COUPON'), null, null, null, null, 0, 0);
        }
        ...
        for ($i = 0; $i < $sixtyPercent; $i++) {
            CouponRepo::createBasicOveralCoupon(config('constants.EVENTS.COUPON').'-'.$i.'60', 60,
                config('constants.EVENTS.COUPON'), null, null, null, null, 0, 0);
        }
        ...
        for ($i = 0; $i < $seventyPercent; $i++) {
            CouponRepo::createBasicOveralCoupon(config('constants.EVENTS.COUPON').'-'.$i.'70', 70,
                config('constants.EVENTS.COUPON'), null, null, null, null, 0, 0);
        }
        ...
        for ($i = 0; $i < $oneHundredPercent; $i++) {
            CouponRepo::createBasicOveralCoupon(config('constants.EVENTS.COUPON').'-'.$i.'100', 100,
                config('constants.EVENTS.COUPON'), null, null, null, null, 0, 0);
        }
    }
}
